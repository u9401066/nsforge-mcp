...
| 2026-07-08 | **生產級強化（tool 收斂＋打包＋可觀測性）** | 用戶問「89→91 需收斂嗎？作為生產級 MCP 還能改哪些」。判斷：91 多數是單一職責、可自描述的獨立符號運算，硬併成 mega-tool 反傷可發現性與自描述原則——故不併；真正該收斂的是任務外圍的 `music`（9）。三塊：(1) **精簡預設面**——`config.py` `module_enabled` 讓 `music` opt-in（`NSFORGE_ENABLE_MUSIC=1`），預設 91→82（少工具＝更好工具選擇，MCP 最佳實務）；catalog 仍 91 可發現；manifest 加 `optional_modules`／`default_tool_count`、`nsforge_health` 報 active vs catalog（58c855c）。(2) **manifest 打包進 wheel**——hatch force-include 映射 `docs/agent/capabilities.json`→`nsforge_mcp/capabilities.json`，`nsforge_manifest`／`health` 於 `uvx` 安裝（無 repo）也可用；meta 解析 repo→packaged；建 wheel 實測含之（300758d）。(3) **可觀測性**——server stderr 結構化啟動日誌（stdout 留給 stdio 協定）＋`NSFORGE_LOG_LEVEL` ＋ `website_url`／版本 metadata（ea1b66f）。selfcheck 亦守衛 `nsforge_mcp.__version__`（第三份版本）與 optional metadata。全程 harness 11/11。未做：跨 91 工具的統一 error envelope（大／風險高，列未來）。 |
