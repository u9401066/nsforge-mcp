...
> Date: 2026-08-31  
> Status: Implemented
> Target: NSForge 0.4.0 / MCP Python SDK 2.1.1 (exact pin)
...
3. **路徑不可逃逸**：repository category、artifact name 與 export path 經正規化及 containment 檢查；遠端 profile 只可寫 artifact root，music artifact root 於工具註冊時凍結。
...
10. **可觀測但不洩密**：tool／session／run correlation id 進入 OpenTelemetry
    spans／events；stderr 只有獨立的啟動 logs。不記錄完整 expression、code、
    token 或 artifact bytes。
...
| `workflow` | agent 的建議精簡面 | 17 個 canonical workflow tools；讀取改走 resources |
| `scientific` | 無 session 的符號計算／驗證 | 35 個 calculate、simplify、verify、expression 嚴格核心 |
| `interactive` | 人機步進推導 | 35 個；workflow 加 session 編輯、handoff 與人工輸入，後者永遠 untrusted |
...
- 產碼：`task_run`／`task_explore` 通過 trusted gate 後，由
  `internal:render_pseudocode` 與 `internal:generate_python_function` phase producers
  物化 artifacts；compact surface 不宣告不存在的公開 compiler tool。
...
- 在 common output schema 保留／宣告 optional `execution_status`、
  `verification_status`、`run_id`、`correlation_id` 與 `resources`；workflow
  `task_run`／`task_explore` 實際回傳這些欄位，其他 legacy payload 不偽造狀態。
  既有 `success`／`verified` 欄位在 legacy payload 保留。
...
任何條件不符皆不得產出 code artifact；實際
`verification_status` 依 outcome 可為 `failed`、`error`、`not_checked` 或 `blocked`。

## 6. State、lifecycle 與執行

Domain 列舉完整 lifecycle 語彙：`draft → running → verifying → verified |
rejected → materialized → completed`，另可進入 `cancelled`／`failed`。v0.4 的實作
透過 ordered phase events 保留過程，並在完成邊界將 final run bundle 單一
transaction 持久化；不宣稱每個中間 status 都已獨立 commit 一個 revision。
Terminal revision 不可再 mutation；修改須 fork 新 revision。
...
- `nsforge://sessions/{session_id}`：workflow 所需的 detached legacy-session snapshot。
- `nsforge://artifacts/{sha256}`：immutable artifact bytes／text；`ResourceLink` 帶 MIME 與
  immutable identity metadata，不額外宣稱 per-resource cache hint。
...
| 規格 | v0.4.0 落點 |
|---|---|
| No-eval parser | 單一 allowlisted token transformation + AST constructor；除長度／深度／power-tower／AST-node budgets，也限制 eager literal exponent >10000、組合／gamma-family special functions 的大整數參數，以及 `polygamma` 更嚴格的 128 上限 |
| Containment | repository category 與 music／artifact output 的 root containment／symlink escape regression；music root 在註冊時凍結，env drift 不改變輸出邊界 |
| Profiles | legacy 82、workflow 17、scientific 35、interactive 35、full 91；啟動凍結 |
| Strict contract | unknown-field rejection + 高風險 enum／range constraints + concise `ToolSpec` descriptions |
| Provenance kernel | immutable run／event／node／evidence／artifact domain objects 與 fail-closed codegen eligibility |
| State | application `RunStore` port + SQLite WAL／foreign keys／optimistic revision Unit of Work |
| MCP primitives | run／event／artifact templates、`ResourceLink`、updated notification、phase progress |
| Observability | SDK OTel spans 的 tool／session／run correlation 與 phase events，不放 sensitive payload |
| Release proof | 14 gates，新增 `security` 與 isolated `package` smoke |

為了不假裝比實際更完整，目前邊界明確如下：

- Legacy `DerivationSession`、current-session fallback 與 YAML repository 仍是
  process-global／JSON／YAML 相容層；strict workflow 才以 SQLite 為權威 run store。
- Tenant id 來自 trusted server configuration，不是 client 可覆寫參數；但未接 IdP／
  token verifier 前，一個 instance 仍只能當一個 tenant trust boundary。
- SQLite／artifact bytes 沒有 cross-replica sharing；水平擴展仍需 shared backend 與
  distributed coordination。
- 取消 `asyncio.to_thread` 的 await 不能強殺已開始的 worker；本版因此不誤發
  Finished，需 hard timeout 時使用 process runner。該隔離路徑會保留相同 events，
  但 progress 在 process 回傳後才重播；live phase stream 限預設無 timeout 路徑。
- SDK 2.1.1 沒有 MCP Tasks implementation；`task_run` 是 NSForge tool，不是
  Tasks extension。
