...
class DrugProperties:
    ...
    name: str
    log_p: float  # 脂水分配係數
    vd_reference: float | None = None  # 文獻參考 Vd (L/kg)
...
class PhysiologicalVdModel:
    ...
    def calculate_vd(self, drug: DrugProperties) -> dict:
        ...
        # PBPK 公式
        vd_total = self.v_plasma + kp_lean * self.v_lean + kp_fat * self.v_fat
    ...
    def adjust_for_body_composition(
        self, drug: DrugProperties, reference_f_fat: float = 0.22
    ) -> dict:
        """
        體組成調整（推薦用法）✅
...
def quick_vd_calculation(weight: float, age: int, sex: int, bmi: float, drug_name: str) -> dict:
    """
    快速計算 Vd（直接預測）
...
def quick_vd_adjustment(weight: float, age: int, sex: int, bmi: float, drug_name: str) -> dict:
    """
    快速體組成調整（推薦用法）
...
def demo():
    ...
    result = quick_vd_calculation(weight=85, age=50, sex=1, bmi=27, drug_name="propofol")
...
""")
    ...
    for label, weight, age, sex, bmi in patients:
        ...
        print(
            f"{label:<8} {weight:>5}kg {bmi:>5} {result['f_fat'] * 100:>6.1f}% "
    ...
    for drug in DRUGS.values():
        ...
        print(
            f"{drug.name:<12} {drug.log_p:>5.1f} {result['kp_fat']:>7.2f} "
